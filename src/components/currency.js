...
export const CurrencyContainerStylesMixin = css`
    ...
    border: none;
    color: ${({ theme: { colors: { heading }}}) => heading};
    padding: 0.5rem 1rem;
...
export const CurrencyWithLogo = ({ logoImageUrl, currentCurrency, showDropdownIcon }) => (
  <>
    <Logo hideLogoOnMobile={showDropdownIcon} imageUrl={logoImageUrl} /><StyledCurrencyLabel showDropdownIcon={showDropdownIcon}>{currentCurrency}</StyledCurrencyLabel>{showDropdownIcon ? <DropdownIcon /> : null}
  </>
)
